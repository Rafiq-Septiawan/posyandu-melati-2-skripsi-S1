...
@section('title', 'Tambah Jadwal Posyandu — Posyandu Melati 2')

@section('content')
<div class="page-header">
    <div class="page-header-left">
        <h2>Tambah Jadwal Posyandu Baru</h2>
        <p>Isi formulir di bawah ini untuk mengumumkan jadwal pelaksanaan posyandu mendatang.</p>
    </div>
...
<div class="card">
    <div class="card-header">
        <i class="fas fa-calendar-plus" style="color: var(--teal-600);"></i>
        <div class="card-title">Formulir Tambah Jadwal</div>
    </div>
    
    <form action="{{ route('ketua.jadwal-posyandu.store') }}" method="POST">
        @csrf
        ...
        <div class="form-section">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="judul" name="judul" class="form-control" placeholder="Contoh: Posyandu Balita & Ibu Hamil Rutin" value="{{ old('judul') }}" required>
                    @error('judul')
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control" required>
                        <option value="aktif" {{ old('status') === 'aktif' ? 'selected' : '' }}>Aktif (Diumumkan)</option>
            ...
            <div class="form-row-3" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                ...
                <div class="form-group">
                    ...
                    <input type="time" id="jam" name="jam" class="form-control" value="{{ old('jam', '08:00') }}" required>
                    @error('jam')
                        <div class="form-hint" style="color: var(--rose-500);">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" id="lokasi" name="lokasi" class="form-control" placeholder="Contoh: Kantor RW 02" value="{{ old('lokasi') }}" required>
                    @error('lokasi')
                        <div class="form-hint" style="color: var(--rose-500);">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-row-1">
                <div class="form-group">
                    ...
                    <textarea id="keterangan" name="keterangan" class="form-control" rows="4" placeholder="Contoh: Harap membawa buku KIA dan fotokopi KK bagi peserta baru.">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <div class="form-hint" style="color: var(--rose-500);">{{ $message }}</div>
                    @enderror
                </div>
