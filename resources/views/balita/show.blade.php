...
@php
  $usiaBulan    = abs((int)\Carbon\Carbon::parse($balita->tanggal_lahir)->diffInMonths(now(), false));
  $usiaLabel    = $usiaBulan >= 12 ? floor($usiaBulan/12).' th '.($usiaBulan%12).' bln' : $usiaBulan.' bulan';
  $isBoy        = $balita->jenis_kelamin === 'L';
  $lastLanjutan = $balita->pemeriksaanLanjutan->sortByDesc('tanggal_periksa')->first();
  $lastAwal     = $lastLanjutan?->pemeriksaanAwal;
  $statusGizi   = $lastLanjutan->status_gizi ?? null;
  $tindakLanjutTerakhir = $lastLanjutan->tindak_lanjut ?? null;

  $sgMap = [
    'gizi baik'   => ['Gizi Baik',   '#22c55e'],
    'gizi normal' => ['Gizi Normal',  '#22c55e'],
    'gizi kurang' => ['Gizi Kurang',  '#f59e0b'],
    'gizi buruk'  => ['Gizi Buruk',   '#ef4444'],
    'stunting'    => ['Stunting',      '#ef4444'],
    'gizi lebih'  => ['Gizi Lebih',   '#3b82f6'],
  ];
  $sgNorm     = strtolower(trim($statusGizi ?? ''));
  $giziLabel  = $sgMap[$sgNorm][0] ?? ($statusGizi ? ucwords($statusGizi) : '-');
  $giziColor  = $sgMap[$sgNorm][1] ?? '#94a3b8';

if ($sgNorm === 'gizi buruk' || $sgNorm === 'stunting') {
...
} elseif ($sgNorm === 'gizi baik' || $sgNorm === 'gizi normal') {
  ...
  } elseif ($tindakLanjutTerakhir === 'rujukan_rs' || $tindakLanjutTerakhir === 'rawat_inap') {
    $bannerGradient = 'linear-gradient(135deg, #dc2626 0%, #b91c1c 50%, #991b1b 100%)';
    $bannerAccent   = 'rgba(252,165,165,0.15)';
  ...
  $sudahDiubah = $balita->updated_at->ne($balita->created_at) && isset($balita->updatedBy);
@endphp
...
<div class="pasien-header" style="background:{{ $bannerGradient }};margin-bottom:20px;position:relative;overflow:hidden;">
  ...
  <div style="flex:1;min-width:0;position:relative;z-index:1;">
    ...
    <div class="pasien-meta" style="color:rgba(255,255,255,0.75);">
      ...
      <span><i class="fas fa-birthday-cake" style="margin-right:4px;"></i>{{ \Carbon\Carbon::parse($balita->tanggal_lahir)->translatedFormat('d F Y') }}</span>
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;position:relative;z-index:1;">
    @foreach([
      ['label'=>'Usia',         'val'=>$usiaLabel,                                                               'icon'=>'fa-cake-candles'],
      ['label'=>'Tanggal Lahir','val'=>\Carbon\Carbon::parse($balita->tanggal_lahir)->translatedFormat('d M Y'), 'icon'=>'fa-calendar'],
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
  ...
  <div class="card">
    <div class="card-header">
      <div class="card-title"><i class="fas fa-id-card" style="color:#3b82f6;margin-right:8px;"></i>Data Pribadi</div>
    </div>
    <div style="padding:16px;">
      @foreach([
        ['Nama Lengkap',  $balita->nama_balita],
        ['NIK',           $balita->nik ?? '-'],
        ...
        ['NIK Ibu',       $balita->ibuHamil->nik ?? '-'],
        ['No. HP Ibu',    $balita->ibuHamil->no_hp ?? '-'],
        ['Alamat Ibu',    $balita->ibuHamil->alamat ?? '-'],
        ['Terdaftar',     $balita->created_at->translatedFormat('d F Y')],
      ] as [$label, $val])
  ...
  <div class="card">
    <div class="card-header">
      <div class="card-title"><i class="fas fa-chart-line" style="color:#22c55e;margin-right:8px;"></i>Data Pertumbuhan Terakhir</div>
    </div>
    <div style="padding:16px;">
      @if($lastLanjutan)
        @php
          $vitLabel = match($lastLanjutan->vitamin_a) {
            'diberikan'       => 'Diberikan',
            'tidak_diberikan' => 'Tidak Diberikan',
            'belum_waktunya'  => 'Belum Waktunya',
            default           => '-',
          };
          $tlLabel = match($lastLanjutan->tindak_lanjut) {
        ...
        @endphp
        @foreach([
          ['Tanggal Periksa', $lastAwal ? \Carbon\Carbon::parse($lastAwal->tanggal_periksa)->translatedFormat('d F Y') : '-', null],
          ['Usia Balita',     ($lastAwal->usia_balita ?? '-').' bulan', null],
          ['Berat Badan',     ($lastAwal->berat_badan ?? '-').' kg', null],
          ['Tinggi Badan',    ($lastAwal->tinggi_badan ?? '-').' cm', null],
          ['Lingkar Kepala',  ($lastAwal->lingkar_kepala ?? '-').' cm', null],
          ['Lingkar Lengan',  ($lastAwal->lingkar_lengan ?? '-').' cm', null],
          ['Status Gizi',     $giziLabel, $giziColor],
          ['Imunisasi',       $lastLanjutan->imunisasi ?? '-', null],
          ['Vitamin A',       $vitLabel, null],
          ['Tindak Lanjut',   $tlLabel, null],
          ['Catatan Bidan',   $lastLanjutan->catatan_bidan ?? '-', null],
        ] as [$label, $val, $color])
        <div style="display:flex;gap:12px;padding:8px 0;border-bottom:1px solid #f1f5f9;">
          <span style="font-size:12px;color:#94a3b8;width:130px;flex-shrink:0;">{{ $label }}</span>
          <span style="font-size:13px;font-weight:600;color:{{ $color ?? '#334155' }};flex:1;">{{ $val }}</span>
        </div>
        @endforeach
      @else
...
<div class="card" style="margin-bottom:16px;">
  ...
  <div style="padding:20px;display:flex;flex-direction:column;gap:12px;">
    ...
    @if(isset($balita->createdBy) && $balita->createdBy)
    <div style="display:flex;align-items:center;gap:14px;padding:14px;background:#f0fdfa;border:1.5px solid #99f6e4;border-radius:12px;">
      <div style="width:44px;height:44px;background:linear-gradient(135deg,#0d9488,#0f766e);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#fff;flex-shrink:0;">
        {{ strtoupper(substr($balita->createdBy->nama ?? $balita->createdBy->name ?? 'A', 0, 1)) }}
      </div>
      <div>
        <div style="font-weight:700;color:#0f172a;font-size:14px;">
          <span style="font-size:11px;font-weight:600;color:#0f766e;">{{ $balita->createdBy->role === 'ketua' ? 'Ketua Posyandu' : ucfirst($balita->createdBy->role ?? 'Pengguna') }}:</span>
          {{ $balita->createdBy->nama ?? $balita->createdBy->name ?? '-' }}
        </div>
        <div style="font-size:12px;color:#334155;margin-top:3px;">
          <i class="fas fa-clock" style="margin-right:3px;"></i>
          {{ $balita->created_at->translatedFormat('d F Y — H:i') }} WIB
        </div>
      </div>
    </div>
    @else
    <div style="display:flex;align-items:center;gap:12px;padding:14px;background:#f0fdfa;border:1.5px solid #99f6e4;border-radius:12px;">
      ...
      <div>
        <div style="font-weight:700;color:#0f172a;font-size:14px;">
          <span style="font-size:11px;font-weight:600;color:#0f766e;">Ketua Posyandu:</span>
          Ketua Posyandu / Sistem
        </div>
        <div style="font-size:12px;color:#334155;margin-top:3px;">
          <i class="fas fa-clock" style="margin-right:3px;"></i>
          {{ $balita->created_at->translatedFormat('d F Y — H:i') }} WIB
    ...
    <div style="display:flex;align-items:center;gap:14px;padding:14px;background:#fffbeb;border:1.5px solid #fcd34d;border-radius:12px;">
      <div style="width:44px;height:44px;background:linear-gradient(135deg,#f59e0b,#d97706);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#fff;flex-shrink:0;">
        {{ strtoupper(substr($balita->updatedBy->nama ?? $balita->updatedBy->name ?? 'U', 0, 1)) }}
      </div>
      <div>
        <div style="font-weight:700;color:#78350f;font-size:14px;">
          <span style="font-size:11px;font-weight:600;color:#d97706;">{{ ucfirst($balita->updatedBy->role ?? 'Pengguna') }}:</span>
          {{ $balita->updatedBy->nama ?? $balita->updatedBy->name ?? '-' }}
        </div>
        <div style="font-size:12px;color:#92400e;margin-top:3px;">
          <i class="fas fa-clock" style="margin-right:3px;"></i>
          {{ $balita->updated_at->translatedFormat('d F Y — H:i') }} WIB
        </div>
      </div>
    </div>
    @else
    <div style="display:flex;align-items:center;gap:12px;padding:14px;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:12px;">
      ...
      <div>
        <div style="font-size:13px;font-weight:600;color:#334155;">Belum pernah diubah</div>
...
<div class="card">
  <div class="card-header">
    <div>
      ...
      <div class="card-subtitle">{{ $balita->pemeriksaanLanjutan->count() }} data</div>
    </div>
  </div>
  @if($balita->pemeriksaanLanjutan->count())
  <div class="table-scroll">
  <table class="dtable">
    <thead>
      <tr>
        <th>No</th><th>Tanggal</th>
        <th>Status Gizi</th><th>Imunisasi</th>
        <th>Vitamin A</th><th>Tindak Lanjut</th><th>Bidan</th>
      </tr>
    </thead>
    <tbody>
      @foreach($balita->pemeriksaanLanjutan->sortBy('tanggal_periksa') as $i => $p)
      @php
        $sgMapRow = [
          'gizi baik'   => ['Gizi Baik',   '#22c55e'],
          'gizi normal' => ['Gizi Normal',  '#22c55e'],
        ...
        $sgRowNorm = strtolower(trim($p->status_gizi ?? ''));
        $gl = $sgMapRow[$sgRowNorm][0] ?? ($p->status_gizi ? ucwords($p->status_gizi) : '-');
        $gc = $sgMapRow[$sgRowNorm][1] ?? '#94a3b8';
        $vitLabel = match($p->vitamin_a) {
          'diberikan'       => 'Diberikan',
          'tidak_diberikan' => 'Tidak Diberikan',
          'belum_waktunya'  => 'Belum Waktunya',
          default           => '-',
      ...
      <tr>
        ...
        <td>
          @if($p->status_gizi)
            <span style="background:{{ $gc }}18;color:{{ $gc }};padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;white-space:nowrap;">{{ $gl }}</span>
          @else - @endif
        </td>
        <td style="font-size:12px;max-width:120px;">{{ $p->imunisasi ?? '-' }}</td>
        <td>{{ $vitLabel }}</td>
        <td>
